...
// --- Storage Service ---
// Uses localStorage for persistence on GitHub Pages (static hosting)
// Falls back to fetching database.json for initial data
...
async function loadData() {
    ...
    // If no localStorage data, fetch from database.json
    try {
        // Get the base path for GitHub Pages compatibility
        const basePath = window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/') + 1);
        const dbUrl = basePath + "database.json";
        ...
        if (!response.ok) {
            throw new Error(`Failed to load database.json (HTTP ${response.status})`);
        }
        ...
        if (Array.isArray(data)) {
            ...
            console.log("Loaded data from database.json and saved to localStorage");
            renderApp();
        } else {
...
function saveData() {
    // Save to localStorage (works on GitHub Pages static hosting)
    try {
        ...
        if (header) {
            ...
            const existing = header.querySelector('.save-status');
